 ...
 *
 * The seeded UNILAG student is given an application at FUL for the length of
...
const FUL = 'http://fulokoja.localhost:3000';
...
test.beforeAll(async () => {
  ...
  await db`DELETE FROM memberships WHERE user_id = ${userId} AND institution_id = ${fulId}`;
  await db`INSERT INTO memberships (user_id, institution_id, role) VALUES (${userId}, ${fulId}, 'candidate')`;
...
test.describe('AU-10', () => {
  test('signing in with two affiliations asks which, with a card for each', async ({ page }) => {
    ...
    const ful = page.locator('li').filter({ hasText: 'Federal University Lokoja' });
    await expect(ful).toContainText('Applicant');
    await expect(ful).toContainText('Applying');
  });
  ...
  test('continuing moves the session to the other host, and back again', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Continue to Federal University Lokoja/ }).click();
    await page.waitForURL(/fulokoja.localhost:3000\/apply/, { timeout: 30_000 });
    ...
    await page.waitForURL(/fulokoja.localhost:3000\/choose-institution/);
    await expect(page.locator('li').filter({ hasText: 'Federal University Lokoja' })).toContainText('You are here');
  ...
  test('a switch token works once, and only at the institution it names', async ({ page, browser }) => {
    ...
    await page.getByRole('button', { name: /Continue to Federal University Lokoja/ }).click();
    await page.waitForURL(/fulokoja.localhost:3000\/apply/, { timeout: 30_000 });
    expect(switchUrl).toContain('fulokoja.localhost');
    ...
    // A live, unspent token for FUL, presented at UNILAG, is refused there,
    ...
    await db`
      ...
      VALUES (${userId}, 'institution_switch', ${createHash('sha256').update(token).digest('hex')},
        ${fulId}, true, now() + interval '60 seconds')`;
    await db.end();
    await sp.goto(`http://unilag.localhost:3000/switch?token=${token}`);
    await expect(sp).toHaveURL(/\/login/);
    await sp.goto(`${FUL}/switch?token=${token}`);
    await expect(sp).toHaveURL(`${FUL}/apply`);
  ...
  test('a default skips the question and goes straight there', async ({ page }) => {
    await logIn(page);
    await page.waitForURL(/\/choose-institution/, { timeout: 30_000 });
    await page.getByRole('button', { name: /Set Federal University Lokoja.* as default/ }).click();
    await expect(page.getByText('Default saved')).toBeVisible({ timeout: 30_000 });
    await expect(page.locator('li').filter({ hasText: 'Federal University Lokoja' })).toContainText('Default');

    // Signing in at UNILAG now lands at FUL, without the chooser.
    await page.context().clearCookies();
    await logIn(page);
    await page.waitForURL(`${FUL}/apply`, { timeout: 30_000 });

    // And it can be undone.
    await page.goto(`${FUL}/choose-institution`);
    await page.getByRole('button', { name: /Stop using Federal University Lokoja.* as default/ }).click();
